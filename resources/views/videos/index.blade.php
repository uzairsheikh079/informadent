                                            ...
                                            <span class="bmd-form-group bmd-form-group-sm">
                                                <form action="{{ route('videos.index') }}" method="GET"
                                                    ...
                                                    <input type="search" class="form-control form-control-sm"
                                                        name="video_name" id="video_name" placeholder="Search videos"
                                ...
                                <div class="col-sm-12">
                                    <table id="datatables" class="table table-striped table-no-bordered table-hover dataTable no-footer dtr-inline" role="grid" aria-describedby="datatables_info" class="table table-hover">
                                        ...
                                        <tr role="row">
                                            ...
                                            <th style="font-weight:bold">Video Name</th>
                                            <th style="font-weight:bold">Video Title</th>
                                            <th style="font-weight:bold">Video Category</th>
                                    ...
                                    @foreach ($videos as $key => $video)
                                          <tr role="row" class="odd">
                                            <td>{{ $video->id }}</td>
                                            <td>{{ $video->user->user_lastname }}, {{ $video->user->user_firstname }}</td>
                                            <td>{{ $video->video_name }}</td>
                                            <td>{{ $video->video_title }}</td>
                                            <td>{{ $video->video_category }}</td>
                                            <!-- <td>{!! Str::substr($video->privately_insured_cuecard_description, 0, 10) !!}...</td>
                                            <td>{!! Str::substr($video->privately_insured_patient_short_description, 0, 10) !!}...</td>
                                            <td>{!! Str::substr($video->privately_insured_patient_long_description, 0, 10) !!}...</td>
                                            <td>{!! Str::substr($video->privately_insured_doctor_long_description, 0, 10) !!}...</td>
                                            <td>{!! Str::substr($video->legaly_insured_cuecard_description, 0, 10) !!}...</td>
                                            <td>{!! Str::substr($video->legaly_insured_patient_short_description, 0, 10) !!}...</td>
                                            <td>{!! Str::substr($video->legaly_insured_patient_long_description, 0, 10) !!}...</td>
                                            <td>{!! Str::substr($video->legaly_insured_doctor_long_description, 0, 10) !!}...</td> -->
                                            <td>
                                            <a href="{{ $video->video_url }}">
                                            <video style="border-radius: 8px;" width="150" controls>
                                            ...
                                            <form action="{{ route('videos.destroy', $video->id) }}" method="POST">
                                                <a type="button" rel="tooltip" title="Show" href="{{ route('videos.show', $video->id) }}" class="btn btn-warning glyphicon glyphicon glyphicon-eye-open">
                                                <i class="material-icons">visibility</i>
                                              </a>
                                                <a type="button" rel="tooltip" title="Edit" href="{{ route('videos.edit', $video->id) }}" class="btn btn-info glyphicon glyphicon glyphicon-eye-open">
