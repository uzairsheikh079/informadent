            ...
            <div class="col-md-12">
                ...
                <div class="card">
                    <div class="card-header card-header-rose card-header-icon">
                        ...
                        <h4 class="card-title">Checkpoints</h4>
                    </div>
                    <div class="card-body">
                        <div class="row">
                            <div class="col-12 text-right">
                                <a href="{{ route('checkpoints.create') }}" class="btn btn-sm btn-rose">ADD CHECKPOINT</a>
                            </div>
                        </div>
                        <div class="row">
                                <div class="col-md-6">
                                    <div class="dataTables_lenght" id="datatables_length">
                                        <label >Showing
                                        <form action="{{ route('checkpoints.index') }}" method="GET" role="search">
                                    ...
                                    <div id="datatables_filter" class="dataTables_filter"> 
                                        <label>
                                            <span class="bmd-form-group bmd-form-group-sm">
                                                <form action="{{ route('checkpoints.index') }}" method="GET"
                                                    role="search" id="search_form">
                                                    <span style="visibility: hidden;">space</span>
                                                    <input type="search" class="form-control form-control-sm"
                                                        name="checkpoint_name" id="checkpoint_name" placeholder="Search checkpoints"
                                ...
                                <div class="col-sm-12">
                                    ...
                                    <table id="datatables" class="table table-striped table-no-bordered table-hover dataTable no-footer dtr-inline" role="grid" aria-describedby="datatables_info" class="table table-hover">
                                        ...
                                        <tr role="row">
                                            ...
                                            <th style="font-weight:bold">Checkpoint</th>
                                    ...
                                    @foreach ($checkpoints as $key => $checkpoint)
                                          <tr role="row" class="odd">
                                            <td>{{ $checkpoint->id }}</td>
                                            <td>{{ $checkpoint->user->user_lastname }}, {{ $checkpoint->user->user_firstname }}</td>
                                            <td>{{ $checkpoint->checkpoint_name }}</td>
                                            <!-- <td>{!! Str::substr($checkpoint->privately_insured_cuecard_description, 0, 10) !!}...</td>
                                            <td>{!! Str::substr($checkpoint->privately_insured_patient_short_description, 0, 10) !!}...</td>
                                            <td>{!! Str::substr($checkpoint->privately_insured_patient_long_description, 0, 10) !!}...</td>
                                            <td>{!! Str::substr($checkpoint->privately_insured_doctor_long_description, 0, 10) !!}...</td>
                                            <td>{!! Str::substr($checkpoint->legaly_insured_cuecard_description, 0, 10) !!}...</td>
                                            <td>{!! Str::substr($checkpoint->legaly_insured_patient_short_description, 0, 10) !!}...</td>
                                            <td>{!! Str::substr($checkpoint->legaly_insured_patient_long_description, 0, 10) !!}...</td>
                                            <td>{!! Str::substr($checkpoint->legaly_insured_doctor_long_description, 0, 10) !!}...</td>
                                            </td> -->
                                            <td class="td-actions text-right">
                                                <form action="{{ route('checkpoints.destroy', $checkpoint->id) }}" method="POST">
                                                    <a type="button" rel="tooltip" title="Show" href="{{ route('checkpoints.show', $checkpoint->id) }}" class="btn btn-warning glyphicon glyphicon glyphicon-eye-open">
                                                    <i class="material-icons">visibility</i>
                                                  </a>
                                                    <a type="button" rel="tooltip" title="Edit" href="{{ route('checkpoints.edit', $checkpoint->id) }}" class="btn btn-info glyphicon glyphicon glyphicon-eye-open">
...
<script> 
    function submitForm() {
        var searchData = $("#checkpoint_name").val();
